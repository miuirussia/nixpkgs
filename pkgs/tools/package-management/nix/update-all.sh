...
for name in $nix_versions; do
    ...
    nix-update --override-filename "$SCRIPT_DIR/default.nix" --version-regex "(2\\.${minor_version}\..+)" --build || true
done
...
nix-update --override-filename "$SCRIPT_DIR/default.nix" --version branch --build
